...
const data = new SlashCommandBuilder()
  ...
  .addSubcommand((subcommand) =>
    subcommand
      ...
      .addStringOption((option) =>
        option.setName("searchterms").setDescription("the song keywords").setRequired(true)
      )
  );
...
const createEmbedByPlaylist = (playlist) => {
  ...
  embed
    ...
    .setDescription(
      `Added **[${playlist.tracks.length} songs](${playlist.url})** from **[${playlist.title}](${playlist.url})** to the queue!`
    )
...
const playByURL = async ({ interaction }) => {
  ...
  if (!searchResult.playlist) {
    ...
    playlist.tracks.forEach((trk) => {
      trk.requestedBy = interaction.user;
    });
...
const run = async ({ interaction }) => {
  ...
  if (!channel) return interaction.editReply("You must be in a voice channel to use this command!");

  if (interaction.options.getSubcommand() === "song") {
    await playByURL({ interaction });
  } else if (interaction.options.getSubcommand() === "search") {
    await playBySearch({ interaction });
  }
};
